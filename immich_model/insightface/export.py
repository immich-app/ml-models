...
def export(model_name: str, output_dir: Path, cache: bool = True) -> None:
    ...
    pack_dir = _download_pack(model_name, output_dir.parent / ".insightface")
    det_src, rec_src = _find_models(pack_dir)

    print(f"Transforming detection model {det_src}")
    det = transform_detection(ir.load(det_src))
    det_path.parent.mkdir(parents=True, exist_ok=True)
    save_with_external_data(det, det_path)

    print(f"Transforming recognition model {rec_src}")
    rec = transform_recognition(ir.load(rec_src))
    rec_path.parent.mkdir(parents=True, exist_ok=True)
    save_with_external_data(rec, rec_path)


def _download_pack(model_name: str, cache_dir: Path) -> Path:
    """The pack is an input, not an output, so `--no-cache` does not reach it; the URL is an immutable tag."""
    pack_dir = cache_dir / model_name
    if pack_dir.is_dir() and any(pack_dir.glob("**/*.onnx")):
        return pack_dir
    ...
    # ignore_errors: nothing needs the removal to succeed -- extractall overwrites, and a missing tree is
...
def _find_models(pack_dir: Path) -> tuple[Path, Path]:
    """Identify detection vs recognition by graph signature; other pack members match neither."""
    det_path = rec_path = None
    # skip macOS AppleDouble sidecars ("._x.onnx"): xattr metadata, not protobuf
    for path in sorted(p for p in pack_dir.glob("**/*.onnx") if not p.name.startswith("._")):
        graph = ir.load(path).graph
        dims = list(graph.inputs[0].shape or []) if len(graph.inputs) == 1 else []
        if len(graph.outputs) >= 5:
            det_path = path
        elif len(graph.outputs) == 1 and len(dims) == 4 and dims[2] == dims[3] == REC_SIZE:
            rec_path = path
