...
from ..onnx.lowering import DecomposeAttention, DecomposeGelu, PatchEmbedToMatMul, _FoldConstantGatherElements
from ..onnx.transforms import _make_init, _node, clear_cached_annotations
...
def _floatify_pad_mask(model: ir.Model) -> bool:
    ...
    for mask in masks:
        ...
        def const(value: float, name: str) -> ir.Value:
            return _make_init(graph, name, np.array(value, np.float32))

        base = f"{mask.name}_padbias"
        axes = _make_init(graph, f"{base}_axes", np.array([1, 2], np.int64))
        cast = _node("Cast", [tokens], to=int(ir.DataType.FLOAT))
        sub = _node("Sub", [cast.outputs[0], const(float(pad.numpy()), f"{base}_pad")])
        abs_ = _node("Abs", [sub.outputs[0]])
        clip = _node("Clip", [abs_.outputs[0], const(0.0, f"{base}_lo"), const(1.0, f"{base}_hi")])
        keep = _node("Sub", [clip.outputs[0], const(1.0, f"{base}_one")])
        bias = _node("Mul", [keep.outputs[0], const(1.0e4, f"{base}_scale")])
        unsq = _node("Unsqueeze", [bias.outputs[0], axes], out=base)
